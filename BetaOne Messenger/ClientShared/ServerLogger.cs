...
namespace BetaOne
{
    public static class ServerLogger
    {
        ...
        {
            Console.WriteLine(String.Format("[{0} -> {1}] {2}".Pastel(System.Drawing.Color.Gray), from, target, toLog.ToString()));
        }
        ...
        public static void logTraffic(Command toLog, string from = "server", string target = "client")
        {
            ...
            Console.WriteLine(String.Format($"[{from} -> {target}]".Pastel(System.Drawing.Color.Gray) +  $" {toLog.name} {contentSize} {errCode} {reqID}"));
        }
